...
from src.clients.dietly_client import (
    DietlyClient,
    DietlyClientAPIError,
    DietlyNoActivePlanError,
)
...
def categorize_error(error_msg: str) -> ErrorCategory:
    ...
    if any(
        x in error_msg for x in ["503", "502", "500", "504", "timeout", "connection"]
    ):
...
async def process_user_meal_sync(user, sites) -> UserSyncResult:
        ...
        try:
            async with DietlyClient(sites.dietly, user.dietly_credentials) as client:
                try:
                    ...
                    if not api_result:
                        return UserSyncResult(
                            user.name, SyncStatus.NO_MENU, NO_MENU_MEALS_MSG
                        )
                    ...
                    for menu_data, order_company_name in menu_results:
                        if not is_valid_api_response(menu_data):
                            logging.warning(
                                f"Invalid API response for order from {order_company_name}"
                            )
                            continue
                    ...
                    return UserSyncResult(
                        user.name, SyncStatus.SUCCESS, NO_ACTIVE_PLAN_MSG
                    )
                ...
                if await sync_menu_to_fitatu(all_menus, company_name, user, sites):
                    return UserSyncResult(
                        user.name, SyncStatus.SUCCESS, "Menu synced successfully"
                    )
                else:
                    return UserSyncResult(
                        user.name,
    ...
    for attempt in range(RETRY_MAX_ATTEMPTS):
        ...
        if (
            ...
            or result.error_category != ErrorCategory.TRANSIENT
...
async def sync_menu_to_fitatu(
    all_menus: List[Tuple[MenuResponse, str]], company_name: str, user, sites
) -> bool:
    ...
    try:
        ...
        for i, (menu, order_company_name) in enumerate(all_menus):
            ...
            if meal_data["meal_ids"]:
                ...
                for meal_name, product_id in meal_data["meal_ids"].items():
                    ...
                    all_meal_data["meal_weights"][unique_meal_name] = meal_data[
                        "meal_weights"
                    ].get(meal_name, 100)
                    meal_name_mapping[unique_meal_name] = (
                        meal_name  # Map unique name back to original
                    )

                logging.info(
                    f"Processed {len(meal_data['meal_ids'])} meals from order {order_company_name}"
                )
            else:
                logging.warning(
                    f"No valid meals found in order from {order_company_name}"
                )

        if not all_meal_data["meal_ids"]:
            logging.warning(
                f"No valid meals found to sync for {user.name} from any order"
            )
            return False

        success = await fitatu.publish_diet_plan(
            get_current_date_iso(),
            all_meal_data["meal_ids"],
            all_meal_data["meal_weights"],
            MEAL_MAPPING,
            meal_name_mapping,
        )
        ...
        if success:
            logging.info(
                f"Successfully synced {len(all_meal_data['meal_ids'])} total meals to Fitatu for {user.name}"
            )
        return success
...
async def process_menu_meals(menu: MenuResponse, fitatu: FitatuClient) -> dict:
    ...
    for meal in menu.deliveryMenuMeal:
        ...
        if product_id := await fitatu.create_or_find_product(
...
async def main():
    ...
    stats = {
        ...
        "successful_syncs": sum(
            1
            for r in results
            if r.status == SyncStatus.SUCCESS and "synced successfully" in r.message
        ),
        "no_active_plan": sum(
            1
            for r in results
            if r.status == SyncStatus.SUCCESS and NO_ACTIVE_PLAN_MSG in r.message
        ),
    ...
    log_json(
        ...
        if exit_code == EXIT_CODE_PARTIAL_FAILURE
        else "error",
