...
from src.utils.utils import (
    extract_user_id_from_jwt_token,
...
class FitatuClient(BaseAPIClient):
    ...
    def __init__(
        self,
        sites_config: SiteConfiguration,
        credentials: FitatuCredentials,
        brand: str,
    ):
        super().__init__()
        ...
        self.diet_activity_base_url = build_api_url(
            self.api_base, "diet-and-activity-plan"
        )
    ...
    async def search_product_by_name(
        self, name: str, date: str, meal: str = "breakfast"
    ) -> Optional[str]:
        ...
        if not response:
            logging.warning(
                f"Search API returned no data for '{name}' - will create new product"
            )
            return None
        ...
        try:
            ...
            if isinstance(response, dict):
                ...
                logging.warning(
                    f"Unexpected search response format for '{name}': {type(response)}"
                )
                return None
                ...
                if isinstance(product, dict):
                    ...
                    if product_name == name and product_id:
                        ...
                        if product_brand == self.brand:
                            ...
                            logging.info(
                                f"Found exact match: '{name}' with brand '{self.brand}' (ID: {product_id})"
                            )
                            ...
                            logging.warning(
                                f"Found product '{name}' with NO BRAND (ID: {product_id}) - will create new product with brand '{self.brand}' instead"
                            )
                        else:
            ...
            if exact_matches:
                ...
                logging.info(
                    f"Using name match: '{name}' with brand '{found_brand}' (ID: {product_id}) - no exact brand match found"
                )
                return str(product_id)
            ...
            if no_brand_matches:
                ...
                logging.warning(
                    f"Found {len(no_brand_matches)} product(s) without brand for '{name}' (ID: {product_id}) - creating new product with brand '{self.brand}' instead"
                )
                return None

            logging.info(
                f"No match found for product: '{name}' - will create new product"
            )
            return None
    ...
    async def get_existing_diet_plan_for_date(self, date: str) -> Dict[str, Any]:
        ...
        return {
            meal_key: [
                ...
                if isinstance(item, dict) and item.get("brand") == self.brand
            ]
    ...
    async def update_diet_plan_for_date(
        self, date: str, diet_plan: Dict[str, Any]
    ) -> bool:
        ...
        logging.debug(
            f"Updating diet plan for {date} with {len(diet_plan.get(date, {}).get('dietPlan', {}))} meal categories"
        )
        ...
        if success:
            ...
            logging.error(
                f"Failed to update diet plan for {date} - no response from API"
            )

        return success

    async def create_or_find_product(
        self, product: NutritionProduct, date: str, meal: str = "breakfast"
    ) -> Optional[str]:
        ...
        if product_id:
            logging.info(
                f"Using existing product '{product.name}' with ID {product_id}"
            )
            return product_id
        ...
        if product_id:
            logging.info(
                f"Found product '{product.name}' in existing diet plan with ID {product_id}"
            )
            return product_id

        # Last resort: Create new product
        logging.info(
            f"Creating new product: '{product.name}' with brand '{product.brand}'"
        )
        ...
        if isinstance(response, dict):
            ...
            if new_product_id:
                logging.debug(
                    f"Successfully created new product '{product.name}' with ID {new_product_id}"
                )
            return new_product_id
        else:
            logging.error(
                f"Failed to create new product '{product.name}' - invalid response format"
            )
            return None

    async def _find_product_in_existing_plan(
        self, product_name: str, date: str
    ) -> Optional[str]:
        ...
        try:
            ...
            if matching_products:
                ...
                logging.debug(
                    f"Found {len(matching_products)} existing products for '{product_name}', using most recent: {most_recent_id}"
                )
            ...
            logging.warning(
                f"Error searching existing diet plan for '{product_name}': {e}"
            )
            return None
    ...
    async def publish_diet_plan(
        ...
        meal_weights: Dict[str, int],
        meal_mapping: Dict[str, str],
        meal_name_mapping: Optional[Dict[str, str]] = None,
    ) -> bool:
        ...
        logging.info(
            f"Existing diet plan contains {len(existing_product_ids)} products"
        )
        ...
        for meal_name, meal_id in meal_ids.items():
            ...
            if not product_exists:  # Only skip if this exact product ID already exists
                logging.debug(
                    f"Adding new meal '{meal_name}' with product ID {meal_id} to diet plan"
                )

                # Validate that the product ID looks reasonable (not empty, not None)
                if not meal_id or str(meal_id).strip() == "":
                    logging.error(
                        f"Skipping '{meal_name}' - invalid product ID: {meal_id}"
                    )
                    continue

                # Use original meal name for mapping if available
                original_meal_name = (
                    meal_name_mapping.get(meal_name, meal_name)
                    if meal_name_mapping
                    else meal_name
                )
                ...
                logging.debug(
                    f"Skipping '{meal_name}' (ID: {meal_id}) - already exists in diet plan"
                )
    ...
    def _add_meal_to_diet_plan(
        ...
        if not (mapped_key := meal_mapping.get(meal_name)):
            logging.info(
                f"Skipping '{meal_name}' - not supported meal by mapping configuration"
            )
            return
        ...
        if mapped_key in existing_plan:
            existing_product_ids = {
                item.get("productId")
                for item in existing_plan[mapped_key]
                if not item.get("deletedAt")
            }
            if meal_id in existing_product_ids:
                logging.info(
                    f"Skipping '{meal_name}' - product {meal_id} already exists in {mapped_key}"
                )
                return
        ...
        logging.info(
            f"Added '{meal_name}' with product ID {meal_id} and brand '{self.brand}' to diet plan ({mapped_key})"
        )
