...
from src.utils.constants import (
    DIETLY_COMMON_HEADERS, USER_AGENT,
    DIETLY_ACTIVE_ORDERS_ENDPOINT, DIETLY_ORDER_DETAILS_ENDPOINT,
    DIETLY_DELIVERY_MENU_ENDPOINT, DIETLY_PROFILE_PATH
)
...
class DietlyClient(BaseAPIClient):
    ...
    def _build_common_headers(self, additional_headers: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    ...
    async def get_order_details(self, order_id: int, company_name: str) -> OrderDetails:
        ...
        url = build_api_url(self.site.base_url, DIETLY_ORDER_DETAILS_ENDPOINT, str(order_id))
        response = await self.get(url)
    ...
    @staticmethod
    async def find_delivery_for_date(order_details: OrderDetails, target_date: str) -> Optional[int]:
    ...
    async def get_delivery_menu(self, delivery_id: int, company_name: str) -> Dict[str, Any]:
        ...
        url = build_api_url(self.site.base_url, DIETLY_DELIVERY_MENU_ENDPOINT, str(delivery_id), "new")
        response = await self.get(url)
