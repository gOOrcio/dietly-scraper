...
def load_configuration_from_file(model: Type[T], file_path: str) -> T:
    ...
    if config_path.suffix in [".yaml", ".yml"]:
        ...
        raise ValueError(
            f"Unsupported configuration file format: {config_path.suffix}. Use .json or .yaml/.yml"
        )
...
class SitesConfiguration(BaseModel):
    ...
    @classmethod
    def load_from_file(
        cls, file_path: str = "config/sites.yaml"
    ) -> "SitesConfiguration":
...
class UserConfiguration(BaseModel):
    ...
    dietly_credentials: DietlyCredentials = Field(
        description="Dietly login credentials"
    )
    fitatu_credentials: FitatuCredentials = Field(
        description="Fitatu login credentials"
    )
...
class UsersConfiguration(BaseModel):
    ...
    @classmethod
    def load_from_file(
        cls, file_path: str = "config/users.yaml"
    ) -> "UsersConfiguration":
